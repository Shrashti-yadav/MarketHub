...
export async function createOrdersTable() {
  try {
    const query = `CREATE TABLE IF NOT EXISTS orders (
      ...
      payment_method VARCHAR(10) DEFAULT 'Card' CHECK (payment_method IN ('Card', 'COD')),
    ...
    // Add column to existing table if it doesn't exist yet
    await database.query(`
      ALTER TABLE orders
      ADD COLUMN IF NOT EXISTS payment_method VARCHAR(10) DEFAULT 'Card'
